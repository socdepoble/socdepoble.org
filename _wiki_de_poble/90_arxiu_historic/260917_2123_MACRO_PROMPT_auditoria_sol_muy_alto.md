...
| Camp | Valor |
| --- | --- |
| Identificador | SDP-PROMPT-202609171923 |
| Versió | 1.0.0 |
| Entorn | entorn-dev-local |
| Creació | 2026-09-17 19:23 |
| Modificació | 2026-09-17 19:23 |
...
4. **CONTENCIÓ ABSOLUTA (PROHIBIT TOCAR CODI):** Tens **ESTRICTAMENT PROHIBIT** modificar, afegir o esborrar cap línia de codi. El teu rol és única i exclusivament d'auditor i analista. Has de generar l'informe amb els defectes trobats; l'agent central (IAIA MarIA) serà qui faça el treball dur de modificar el codi.
...
Hem finalitzat la Fase 2 del nostre projecte, on hem adaptat l'arquitectura d'UI al model `UniversalWorkspace` i netejat profundament el CSS obsolet. Ara ens encarem a la Fase 3: la preparació final.
...
1. **Auditoria "Sol Muy Alto" (Vista d'ocell):** Analitza l'arquitectura global de Sóc de Poble des d'una perspectiva de 10.000 peus. No et fixes en petits errors tipogràfics.
2. **Coherència del Sistema de Disseny (Pedra Seca):** Verifica que el nou model d'`UniversalWorkspace` i la distribució de carpetes compleixen amb els principis establerts.
3. **Punts de fallada i orfandat:** Identifica qualsevol component, script o ruta que estiga aïllat o que puga causar deute tècnic a mitjà termini.
4. **Sistema de Gestió de Qualitat (ISO / Plantilles):** Actualment tenim una plantilla anomenada `00_PLANTILLA_PROMPT_ISO.md`. Volem implementar un autèntic sistema de gestió de qualitat (com les normes ISO) aplicable a totes les nostres plantilles, formularis i elements iterables, per anar millorant-los contínuament. Com enteneu les IAs aquests sistemes de qualitat? Quin és el nom o l'estàndard adequat en enginyeria de programari / IA per a açò? Dissenya'ns o proposa'ns un sistema de qualitat realment utilitzable per a les nostres plantilles, adaptable a tota la Wiki.
5. Elabora un dictamen (en valencià) clar, concís i directe, presentat en format Markdown.
