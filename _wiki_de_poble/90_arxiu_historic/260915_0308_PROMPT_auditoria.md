...
Bundle aparellat: 260915_0308_BUNDLE_auditoria.md.
...
Analitza el codi i la Wiki adjunts per resoldre les següents "vergonyes" de disseny i disfuncions UI que ens han quedat pendents:

1. **Col·lapse del Xat**: La pàgina del Xat (ni la general ni la incrustada) no apareix. La consola llança l'error `Uncaught TypeError: Cannot read properties of undefined (reading 'startTime')` dins de `reportAllChanges` i `n.timeout`. A més, la consola s'omple d'errors `Facet malformat (cal id, options[] i getValue)` des de `UniversalWorkspace.jsx`.
2. **Desquadrament de la Topbar**: La barra blava de navegació superior s'ha desplaçat i no està apegada a la part superior de la pantalla. Hi ha uns 10 píxels de separació (marge) entre el contenidor del navegador i la pàgina web.
3. **Scroll Deficient**: L'scroll general de l'aplicació va molt malament o directament no funciona.
4. **Logotip Invisible en Mode Clar**: El logotip de "Sóc de Poble" del menú lateral esquerre es pinta de color blanc. En mode fosc es veu bé, però en mode clar desapareix perquè és blanc sobre blanc. Ha de ser negre en mode clar.

`EXECUTA: Analitza el codi i la Wiki adjunts, identifica les causes d'aquests problemes d'interfície i proposa les correccions mínimes verificables per retornar la UI a la seua esplendor.`
