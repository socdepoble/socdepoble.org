...
<html lang="ca">
  <head>
    ...
    <!-- CSP estrict: S'ha afegit 'unsafe-inline' temporalment a style-src per desbloquejar el Grid del Shadow DOM i el Pull-to-refresh -->
    <meta http-equiv="Content-Security-Policy" content="default-src 'self' data: blob:; style-src 'self' 'unsafe-inline' https:; script-src 'self' 'unsafe-inline'; connect-src 'self' https: wss: ws://localhost:* http://localhost:*; img-src 'self' data: blob: https:; font-src 'self' https: data:; object-src 'none'; base-uri 'self';">
