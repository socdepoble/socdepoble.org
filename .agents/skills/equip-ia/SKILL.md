...
- **Codex (GPT-4o):** Eficiència extrema per tokens. El seu ús continuat fulmina la bossa setmanal, però s'ha comprovat (set. 26) que el pla Pro permet fins a 40 prompts d'auditoria extrema per cicle de restabliment. És altament complementari a Fable per a balancejar la càrrega. Reservar-lo per a revisions quirúrgiques d'alta complexitat.
...
- **Fable (Gemini Agentic via IDE):** Extrema capacitat d'execució autònoma, però perillós a nivell de consum (Rate Limits) a màxima potència. **Nova Estratègia:** Fer auditories o tasques d'esforç baix consumeix només un 1%, assolint una equivalència tàctica i de cost amb Codex. Són dues IAs perfectament complementàries: alterna entre Fable (esforç baix) i Codex per allargar la resistència del sistema en sessions llargues. L'esforç Alt queda reservat només per a deducció profunda ineludible.
